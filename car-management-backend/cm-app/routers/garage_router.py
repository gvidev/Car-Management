...
@garage_router.get("/dailyAvailabilityReport",
                   response_model=list[GarageDailyAvailabilityReportDTO],
                   responses=responses)
...
@garage_router.get("/{id}", response_model=ResponseGarageDTO, responses=responses)
async def get_single_garage(id: int):
     return get_garage(id)

@garage_router.get("", response_model=list[ResponseGarageDTO], responses=responses)
async def get_all_garages_by_city(city: str | None = None):
     return get_garages(city=city)

@garage_router.post("", response_model=ResponseGarageDTO, responses=responses)
async def create_new_garage(garage: CreateGarageDTO):
     return create_garage(garage)

@garage_router.put("/{id}", response_model=ResponseGarageDTO, responses=responses)
async def update_single_garage(id: int, garage: UpdateGarageDTO):
     return update_garage(id,garage)

@garage_router.delete("/{id}", response_model=bool, responses=responses)
